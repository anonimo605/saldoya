    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            if (querySnapshot.empty) {
                toast({ variant: "destructive", title: "Código Inválido", description: "El código de regalo que ingresaste no existe." });
                return;
            }
            ...
            if (now > expirationDate) {
                toast({ variant: "destructive", title: "Código Expirado", description: "Este código de regalo ya no es válido." });
                return;
            }

            if (giftCode.redeemedBy.length >= giftCode.usageLimit) {
                toast({ variant: "destructive", title: "Código Agotado", description: "Este código de regalo ha alcanzado su límite de usos." });
                return;
            }

            if (giftCode.redeemedBy.includes(user.id)) {
                toast({ variant: "destructive", title: "Ya Canjeado", description: "Ya has canjeado este código de regalo anteriormente." });
                return;
            }
            ...
            await runTransaction(db, async (transaction) => {
                ...
                // Create the transaction record first (outside the atomic update, but within the process)
            ...
            toast({ variant: "destructive", title: "Error", description: "No se pudo canjear el código. Inténtalo de nuevo." });
        }
    };

...
const ReferralSection = () => {
        ...
        const fetchReferredUsers = async () => {
            if (user && user.referredUsers && user.referredUsers.length > 0) {
                ...
                const referrals = querySnapshot.docs.map(doc => {
                    const data = doc.data();
                    return { id: doc.id, displayId: data.displayId, phone: data.phone, balance: data.balance } as ReferredUser;
                });
    ...
    const handleCopy = () => {
        ...
        toast({ title: "¡Copiado!", description: "Tu código de referido ha sido copiado al portapapeles." });
    };
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Invita a tus amigos compartiendo tu código único y visualiza tus referidos aquí.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                <div>
                    <p className="text-sm font-medium mb-2">Tu código de referido:</p>
                    <div className="flex w-full items-center space-x-2">
                        <Input value={referralCode} readOnly placeholder="Generando código..." />
                        <Button variant="secondary" size="icon" onClick={handleCopy} disabled={!referralCode}>
